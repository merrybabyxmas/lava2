...
def build_model(script_args, checkpoint_dir):
    ...
    if not script_args.full_finetune:
        ...
        if checkpoint_dir is not None:
            ...
            # 🔥 수정: named_parameters로 처리
            logger.info("Setting LAVA parameters to trainable...")
            
            trainable_names = []
            for name, param in model.named_parameters():
                # LAVA 파라미터: 'lava'가 이름에 있고 float type
            ...
            if script_args.local_rank == 0:
                logger.info(f"Set {len(trainable_names)} LAVA parameters to trainable")
                logger.info(f"Sample trainable params (first 10):")
                ...
                logger.info(f"  b_mu: {b_mu_count}")
                logger.info(f"  b_logvar: {b_logvar_count}")
                logger.info(f"  W_mu.weight: {w_mu_count}")
                logger.info(f"  W_o.weight: {w_o_weight_count}")
                logger.info(f"  W_o.bias: {w_o_bias_count}")
                logger.info(f"  Total: {len(trainable_names)}")
                
                if w_mu_count == 0 or b_logvar_count == 0:
                    logger.error("❌ CRITICAL: LAVA parameters not properly trainable!")
                    raise ValueError("LAVA parameter setup failed!")
